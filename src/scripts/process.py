...
def main():
    ...
    # Process walk files
    logger.info("Processing walk files...")
    walks_gdf = process_walk_files(RAW_DATA_DIR)
    if walks_gdf.empty:
        logger.error("No valid walks found.")
        sys.exit(1)
    ...
    logger.info(f"Saved {len(walks_gdf)} walks to {walks_file}")
    
    # Load street network
    logger.info("Loading street network...")
    streets_gdf = load_street_network(city)
    if streets_gdf.empty:
        logger.error("No street network could be loaded.")
        sys.exit(1)
    
    # Analyze walks and calculate coverage
    logger.info("Analyzing walks and calculating coverage...")
    ...
    logger.info(f"Saved street coverage data to {coverage_file}")
    ...
    logger.info(f"Total streets: {total_streets}")
    logger.info(f"Covered streets: {len(covered_streets)} ({coverage_percent:.1f}%)")
    ...
    logger.info(f"Total walk distance: {total_distance_km:.1f} km")
